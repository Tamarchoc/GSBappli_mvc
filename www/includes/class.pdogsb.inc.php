...
class PdoGsb {
    ...
    public function getLesVisiteurs($idVisiteur, $mois) {
        $requetePrepare = PdoGsb::$monPdo->prepare(
    ...
    public function majFraisHorsForfait($id, $idVisiteur, $unMois, $date, $libelle, $montant) {
        $dateFr = dateFrancaisVersAnglais($date);
        $requetePrepare = PdoGSB::$monPdo->prepare(
    ...
    public function getMontantFraisForfait($idVisiteur, $mois) {
     ...
     /**
     * Retourne le montant total des frais hors forfait et forfaitisés
     * pour le mois et le visiteur concerné
     *
     * @param String  $idVisiteur      ID du visiteur
     * @param String  $leMois            Mois sous la forme aaaamm
     * @param String $total        Somme FF et FHF
     *
     */
public function majTotal($total,$idvisiteur,$mois){
        $requetePrepare = PdoGSB::$monPdo->prepare(
            'UPDATE fichefrais '
            . 'SET montantvalide = :total,idetat="VA" '
            . 'WHERE idvisiteur = :unIdVisiteur  '
            . 'AND mois = :unMois'
        );
