...
function nbErreurs()
{
    ...
    function getMoisASelectionner() {
    $dateActuelle = new DateTime();  // Obtient la date actuelle
    $lesMois = array();
    

    for ($i = 0; $i < 12; $i++) {
        // Retire un mois à la date actuelle
        $dateActuelle->modify('-1 month');
    
        // Ajoute le mois au tableau
        $lesMois[] = $dateActuelle->format('Y-m');
  }
          return $lesMois;
    }
...
function getNbjustificatifs($idVisiteur, $mois) {
...
function getmontantFraisHorsForfait ($idVisiteur, $mois){
        $requetePrepare->bindParam(':unIdVisiteur', $idVisiteur, PDO::PARAM_STR);
